...
class model_tach_4delays:
    ...
    def __init__(
        ...
        self.full_readout = (
            full_readout  # to use the readout at the end of the processing or at RT
        )
        ...
        self.params_delays = np.array(
            [self.tse_TIED, self.tmotor_TIED, self.tse_PA, self.tmotor_PA]
        )
        self.params_contaminants = np.array([self.b, self.c, self.d])

        self.t_theta = (
            2
            * 10 ** (self.compressive_power * self.ABL / 20)
            / (self.T0 * self.a_tied**2)
        )
    ...
    def set_params_alexis(self, params_alexis):
        # write your code here
        # for example if i wanted to fit the compressive_power, a_tied,  and the T0
        # the input params_alexis would be a list with [compressive_power, a_tied, T0]
        # So the code in the function would be
        # compressive_power, a_tied, T0 = params_alexis
        # self.compressive_power = compressive_power
        # self.a_tied = a_tied
        # self.T0 = T0
        # self.params_alexis = np.array([compressive_power, a_tied, T0])
        #####################################

        tmotor, tse_TIED = params_alexis 
        self.tmotor_TIED = tmotor
        self.tse_PA = tmotor
        self.tse_TIED = tse_TIED
        self.params_alexis = np.array([tmotor, tse_TIED])
    ...
    def acc_in_bounds_raw_withnorm(self, t2):  # returns pr
        ...
        for k in range(K):
            ...
            S1[inds1] += (
                exp(4 * mus * k) * F1p
                + exp(-4 * mus * k) * F1m
                - exp(2 * mus * (2 * (1 - k) - z)) * F2p
                - exp(2 * mus * (2 * (1 + k) - z)) * F2m
            )
            ...
            S2[inds2] += (
                (F1 - F2) * sin(n * pi * z / 2) * exp(-1 / 2 * An(n) * tl)
            )  # z is not vectorized yet
    ...
    def F_RT_pa(self, t2):
        ...
        Phi = lambda x: 0.5 * erfc(-x / sqrt(2))  # this is like normcdf but faster
        Fi = lambda x, mu: Phi(-(1 - mu * x) / sqrt(x)) + exp(2 * mu) * Phi(
            -(1 + mu * x) / sqrt(x)
        )
    ...
    def f_and_F_RT_pa(self, t2):
        ...
        p = (
            np.divide(
                1.0,
                sqrt(2 * pi * t**3, where=t > 0, out=np.zeros_like(t)),
                where=t > 0,
                out=np.zeros_like(t),
            )
            * exp(
                -np.divide(
                    (1 - mu_s * t) ** 2, 2 * t, where=t > 0, out=np.zeros_like(t)
                )
            )
            * t_theta
        )
        ...
        dsqrt = np.divide(
            1,
            sqrt(t, where=t > 0, out=np.zeros_like(t)),
            where=t > 0,
            out=np.zeros_like(t),
        )
        P = np.where(
            t <= 0,
            0,
            Phi(-(1 - mu_s * t) * dsqrt) + exp(2 * mu_s) * Phi(-(1 + mu_s * t) * dsqrt),
        )
    ...
    def pdf_cont(self, t_max):
        ...
        exp_component = (
            self.d
            * self.b
            * np.exp(-self.b * (self.RT + self.t_fix))
            / (1 - np.exp(-self.b * t_c))
        )
        uniform_component = (1 - self.d) / t_c
        cont_pdf = np.where(
            (self.RT + self.t_fix) <= t_c, exp_component + uniform_component, 0
        )
        cont_pdf = np.where((self.RT + self.t_fix) < 0, 0.0, cont_pdf)
        return cont_pdf
    ...
    def cdf_cont(self, t_max):
        ...
        exp_component = (
            self.d
            * (1 - np.exp(-self.b * (self.RT + self.t_fix)))
            / (1 - np.exp(-self.b * t_c))
        )
        ...
        cont_cdf = np.where((self.RT + self.t_fix) < 0, 0.0, cont_cdf)
        return cont_cdf
    ...
    def pdf_and_tach(self):
        ...
        F_pa_max = self.f_and_F_RT_pa(
            np.array(1000) - (self.tmotor_PA - self.tse_PA - self.t_fix)
        )[1]
        # the pdf and cdf of PA model
        pdf_pa = (
            self.f_and_F_RT_pa(self.RT - (self.tmotor_PA - self.tse_PA - self.t_fix))[0]
            / F_pa_max
        )
        cdf_pa = (
            self.f_and_F_RT_pa(self.RT - (self.tmotor_TIED - self.tse_PA - self.t_fix))[
                1
            ]
            / F_pa_max
        )
        ...
        # TIED model
        # to normalize
        F_tied_max = self.f_and_F_RT_all_tied(
            np.array(1000) - self.tmotor_TIED - self.tse_TIED - 10.0
        )[1]
        # the pdf and cdf of tied model
        pdf_tied = (
            self.f_and_F_RT_all_tied(self.RT - self.tmotor_TIED - self.tse_TIED - 10.0)[
                0
            ]
            / F_tied_max
        )
        cdf_tied = (
            self.f_and_F_RT_all_tied(self.RT - self.tmotor_PA - self.tse_TIED - 10.0)[1]
            / F_tied_max
        )
        ...
        if self.full_readout:
            acc_pa_0 = self.acc_in_bounds_RTs_withnorm(self.RT - 10.0, 1)
        else:
            acc_pa_0 = self.acc_in_bounds_RTs_withnorm(
                self.RT - self.tmotor_PA - self.tse - 10.0, 1
            )

        F_tied_min = self.f_and_F_RT_all_tied(
            self.RT - self.tmotor_PA - self.tse_TIED - 10.0
        )[1]
        # whether Delta is being used or not
        if not self.use_Delta:
            F_tied_RT = self.f_and_F_RT_all_tied(self.RT - 10.0)[1]
            F_tied_RT = np.where(self.RT - 10.0 <= 0, 0.0, F_tied_RT)
        else:
            F_tied_RT = self.f_and_F_RT_all_tied(
                self.RT - self.tmotor_PA - self.tse + self.Delta - 10.0
            )[1]
            F_tied_RT = np.where(
                self.RT - self.tmotor_PA - self.tse + self.Delta - 10.0 <= 0,
                0.0,
                F_tied_RT,
            )
        ...
        # add contaminants
        pdf_cont = self.pdf_cont(np.array(2200.0))
        pdf_joint_with_conts = pdf_joint * (1 - self.c) + self.c * pdf_cont
        tach_with_conts = (
            (1 - self.c)
            * (acc_pa * pdf_pa * (1 - cdf_tied) + acc_tied * pdf_tied * (1 - cdf_pa))
            + self.c * 0.5 * pdf_cont
            + 1e-10
        ) / (pdf_joint_with_conts + 2e-10)
    ...
    def pdf_and_tach_for_debug(self):
        """
        Compute joint PDF and tachometric.
        """
        # the total RTD and the tachometric with shift
        ...
        return pdf_joint_with_conts, tach_with_conts, pdf_pa, pdf_tied
