...
df_session = parse('/home/alexis/2AFC/setups/913/sessions/913_stage_training_20210601-162220/913_stage_training_20210601-162220.csv')
...
########################################################################################################################
    ...
    for k in range(len(df_session.Side.unique())):  # k=0 left trials and k=1 right trials
        ...
        if k == 0:  # Left subplot: left trials
            ...
            ax = plt.subplot2grid((16, 4), (0, 0), rowspan=8, colspan=2)
            stim_color = 'tab:blue'
            ...
            ax = plt.subplot2grid((16, 4), (0, 2), rowspan=8, colspan=2)
        ...
        for j in range(len(df_session[df_session.Side == k])):  # n trials
            ...
            ax.barh(df_session[df_session.Side == k].reset_index().index,
                    ...
                    color=stim_color, label='Stim', zorder=1)  # Need to specify zorder otherwise response window is plotted
                # under stimulus length and can't be seen
...
########################################################################################################################
    ...
    for k in range(len(df_session.Side.unique())):  # k=0 left trials and k=1 right trials
        ...
        if k == 0:  # Left subplot: left trials
            # ax = plt.subplot2grid((1, 2), (0, 0))
            ax = plt.subplot2grid((16, 4), (8, 0), rowspan=4, colspan=2)
            # ax.set_title('Left trials')
            #ax.set_xlabel('Time (s)')
            ...
            # ax = plt.subplot2grid((1, 2), (0, 1))
            ax = plt.subplot2grid((16, 4), (8, 2), rowspan=4, colspan=2)
            # ax.set_title('Right trials')
            #ax.set_xlabel('Time (s)')
...
########################################################################################################################
    ...
    for k in range(len(df_session.Side.unique())):  # k=0 left trials and k=1 right trials
        ...
        if k == 0:  # Left subplot: left trials
            # ax = plt.subplot2grid((1, 2), (0, 0))
            ax = plt.subplot2grid((16, 4), (13, 0), rowspan=3, colspan=2)
            # ax.set_title('Left trials')
            ax.set_xlabel('Time (s)')
            ...
            # ax = plt.subplot2grid((1, 2), (0, 1))
            ax = plt.subplot2grid((16, 4), (13, 2), rowspan=3, colspan=2)
            # ax.set_title('Right trials')
            ax.set_xlabel('Time (s)')
            ax.spines['top'].set_visible(False)
            ax.spines['right'].set_visible(False)
...
# Register time again and compute the total runtime of the script
time2 = time.time()
runtime = time2-time1
