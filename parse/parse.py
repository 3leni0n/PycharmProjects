...
# Path to csv (laptop)
# path = '/home/alexis/PycharmProjects/parse/Test_02hero_20210323-211243/Test_02hero_20210323-211243.csv'  # 10 trials no evidences (doesn't work now with parse due to changes)
# path = '/home/alexis/PycharmProjects/parse/Test_02hero_20210323-211918/Test_02hero_20210323-211918.csv'  # 50 trials no evidences (doesn't work now with parse due to changes)
# path = '/home/alexis/PycharmProjects/parse/Test_02hero_20210330-201034/Test_02hero_20210330-201034.csv'  # 30 trials with evidences (mostly misses)
# path = '/home/alexis/PycharmProjects/parse/Test_Untitled task 30_20210407-195249/Test_Untitled task 30_20210407-195249.csv'  # 53 trials stage 4 with evidences
...
index = df[df['TYPE'] == 'TRIAL'].index  # Use this one because after END-TRIAL it comes the summary of the previous one
...
for i in range(len(index) - 1):  # -1 to not take into account last trial
    ...
    stim_start.append(band[(band['TYPE'] == 'STATE') & (band['MSG'] == 'StimulusTrigger')]['BPOD-FINAL-TIME'].iloc[0])
    # stim_end.append(band[(band['TYPE'] == 'STATE') & (band['MSG'] == 'Stop')]['BPOD-FINAL-TIME'].iloc[0])
    stim_end.append(band[(band['TYPE'] == 'STATE') & (band['MSG'] == 'StimulusStop')]['BPOD-FINAL-TIME'].iloc[0])
    ...
    resp_win_end.append(band[(band['TYPE'] == 'STATE') & (band['MSG'] == 'ResponseWindow')]['BPOD-FINAL-TIME'].iloc[0])
...
columns = ['Trial', 'Side', 'RepTrial', 'Reward', 'Punish', 'Miss', 'WrongLick', 'Hit', 'AfterHit', 'Choice',
           ...
           'RespWinStart', 'RespWinEnd', 'RespWinLen', 'Filename', 'Evidence', 'EviRep', 'Coherence', 'Port1In',
           'Port1Out', 'Port2In', 'Port2Out', 'AW', 'Timeout', 'Fixation', 'Stage', 'Substage', 'Motor', 'REC',
...
data = list(zip(trial, reward_side, rep_trial, reward, punish, miss, wrong_lick, hit, after_hit, choice,
                rep_choice, response, trial_start, trial_end, trial_len, stim_start, stim_end, stim_len, resp_win_start,
                resp_win_end, resp_win_len, filename, evidence, evi_rep, coherence, port1in, port1out, port2in,
                port2out, aw, timeout, fixation, stage, substage, motor, rec, serial_port, protocol, creator, project,
                experiment, board, setup, net_port, subject, bpod_api_version, session, date, time_session_started,
                time_session_ended))
